function planTrade(signal,{riskPct=0.5,equity=10000,maxDailyLossPct=2,maxPositionUnits=100000}={}){
  ...
  if(!['LONG','SHORT'].includes(side)||(signal.filters||[]).length||(side==='LONG'?signal.probability:1-signal.probability)<0.62)return {allowed:false,reason:'Research trade filters did not pass'};
