function planTrade(signal,{riskPct=0.5,equity=10000,maxDailyLossPct=2,maxPositionUnits=100000}={}){
  ...
  const minProbability=Math.max(.5,Math.min(.95,Number(signal.minProbability??process.env.SIGNAL_MIN_PROBABILITY??.70)));const directionalProbability=Number(signal.directionalProbability??(side==='LONG'?signal.probability:1-signal.probability));
