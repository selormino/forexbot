...
async function manualTrade(i){
 ...
 try{
   ...
   if(!confirm(`Send manual intent #${created.intent.id} to the configured ${mode.toUpperCase()} broker bridge now?`))return;
...
async function load(){try{const [m,b,h,e]=await Promise.all([get('/api/signals/metrics'),get('/api/signals/board'),get('/api/signals/history?limit=300'),get('/api/research/edge')]);renderAccuracy(m);renderSeries(m);renderBoard(b);renderHistory(h);renderEdge(e);renderBroker();}catch(e){$('signalBoard').innerHTML='<tr><td colspan="11">'+esc(e.message)+'</td></tr>'}}
