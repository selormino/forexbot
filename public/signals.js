...
async function load(){try{const [m,b,h]=await Promise.all([get('/api/signals/metrics'),get('/api/signals/board'),get('/api/signals/history?limit=300')]);renderAccuracy(m);renderSeries(m);renderBoard(b);renderHistory(h);renderBroker();}catch(e){$('signalBoard').innerHTML='<tr><td colspan="11">'+esc(e.message)+'</td></tr>'}}
