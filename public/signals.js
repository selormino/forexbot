...
async function openSetupChart(sig){
 if(!sig)return;
 modal(sig.symbol+' · '+sig.timeframe+' chart analysis','<div class="notice">Loading point-in-time closed candles and indicators…</div>','<button onclick="closeModal()">Close</button>',true);
 try{
  const sourceTs=Number(sig.sourceCandleTs??sig.source_ts);
  const at=Number.isFinite(sourceTs)?'&at='+encodeURIComponent(sourceTs):'';
  const data=await get('/api/signals/chart?symbol='+encodeURIComponent(sig.symbol)+'&timeframe='+encodeURIComponent(sig.timeframe)+'&limit=90'+at);
  const sum=data.indicatorSummary||{},pa=data.priceAction||{},conf=sig.analysis?.confluence||{};
  const asOf=Number.isFinite(Number(data.analysisSourceTs))?new Date(Number(data.analysisSourceTs)).toLocaleString():'latest closed candle';
  $('modalBody').innerHTML='<div class="notice">Chart reconstructed from closed '+esc(sig.timeframe)+' candles through <b>'+esc(asOf)+'</b>.</div><div class="chart-summary"><div><span class="label">Setup probability</span><b>'+pct(sig.setupProbability??sig.setup_probability)+'</b></div><div><span class="label">Evidence</span><b>'+((conf.agreement??'—')+(conf.agreement==null?'':'%'))+'</b></div><div><span class="label">Structure</span><b>'+esc(pa.structure||'—')+'</b></div><div><span class="label">Pattern bias</span><b class="'+(pa.patternBias>0?'long':pa.patternBias<0?'short':'wait')+'">'+Number(pa.patternBias||0).toFixed(2)+'</b></div></div>'+mainChartSvg(data,sig)+'<div class="indicator-stack">'+oscillatorSvg(data.indicators?.rsi||[],{min:0,max:100,lines:[30,50,70],label:'RSI (14)'})+macdSvg(data)+'</div><div class="analysis-grid chart-analysis-grid"><div class="analysis-box"><h3>Indicator interpretation</h3>'+(data.explanations||[]).map(x=>'<p>'+esc(x)+'</p>').join('')+'</div><div class="analysis-box"><h3>Evidence components</h3><p>Technical: '+Number(conf.components?.technical||0).toFixed(2)+'</p><p>Structure: '+Number(conf.components?.structure||0).toFixed(2)+'</p><p>Pattern: '+Number(conf.components?.pattern||0).toFixed(2)+'</p><p>Higher timeframe: '+Number(conf.components?.higherTimeframe||0).toFixed(2)+'</p><p>Fundamental: '+Number(conf.components?.fundamental||0).toFixed(2)+'</p></div><div class="analysis-box"><h3>Current indicators</h3><p>EMA alignment: <b>'+esc(sum.emaAlignment||'—')+'</b></p><p>RSI: <b>'+num(sum.rsi)+'</b> · '+esc(sum.rsiState||'')+'</p><p>MACD: <b>'+esc(sum.macdState||'—')+'</b></p><p>ADX: <b>'+num(sum.adx)+'</b> · '+esc(sum.trendStrength||'')+'</p></div>'+patternCards(data)+'</div>';
 }catch(e){$('modalBody').innerHTML='<div class="notice">'+esc(friendly(e))+'</div>';}
}
window.showChart=i=>openSetupChart(board[i]);
window.showHistoryChart=i=>{
